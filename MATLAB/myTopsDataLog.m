classdef (Sealed) myTopsDataLog < topsGroupedList
   % @class myTopsDataLog
   % One central place to log data on the fly.
   % myTopsDataLog makes it easy to log data as you go.  It's a "singleton"
   % class, which means that only one can exist at a time, and you never
   % have to create one yourself.  You just call myTopsDataLog methods from
   % any code that needs to use the log.
   % @details
   % myTopsDataLog is a subclass of topsGroupedList, which means it
   ...
   % of data because myTopsDataLog uses timestamps to identify pieces of
   % data in each group.
   % @details
   ...
   methods (Access = private)
      ...
      % @details
      % Use myTopsDataLog.theDataLog to access the current instance of
      % myTopsDataLog.
      function self = myTopsDataLog()
         ...
         self.fileWithPath = 'myTopsDataLog.mat';
      end
   end
   ...
   methods (Access = protected)
      ...
      function writeIncrementToFile(self)
         ...
         newData = myTopsDataLog.getSortedDataStruct(newRange);
   ...
   methods (Static)
      ...
      % @details
      % Returns the current instance of myTopsDataLog.  Use this method
      ...
      % @details
      % If @a isVerbose is true, myTopsDataLog will print a message to the
      % Command Window whenever data is added to the log.  If @a
      % isVerbose is false or omitted, myTopsDataLog will print nothing.
      ...
      % @details
      % You can't create new instances of myTopsDataLog, but you can
      ...
      % current instance of myTopsDataLog.  If @a timestamp is omitted,
      % uses the current time reported by clockFunction.
      % @details
      % Updates earliestTime and latestTime to account for the the time
      % of this log entry.
      % @details
      % Since myTopsDataLog is a subclass of topsGroupedList, logging data
      ...
      function logDataInGroup(data, group, timestamp)
         ...
         if self.printLogging
            disp(sprintf('myTopsDataLog: %s', group))
         end
      ...
      % [laterThan asLateAs]
      % @details
      % Gets all data items from the current instance of myTopsDataLog, as
      ...
      % @details
      % Gets recent data items from the current instance of myTopsDataLog,
      ...
      function logStruct = getNewData()
         ...
         if self.lastNewDataTime < self.latestTime;
            ...
            logStruct = myTopsDataLog.getSortedDataStruct(newRange);
      ...
      % myTopsDataLog.getSortedDataStruct() and writes the struct to disk,
      % via the topsDataFile class.
      % @details
      % writeDataFile() behaves differently depending on @a fileWithPath
      % and the fileWithPath property of the current myTopsDataLog
      % instance:
      %   - If @a fileWithPath is provided, assigns @a fileWithPath to
      %   the fileWithPath property of the current myTopsDataLog instance
      %   and writes data to the given @a fileWithPath.
      %   - If @a fileWithPath is omitted, but the current myTopsDataLog
      %   instance has a non-empty fileWithPath property, writes data
      %   according to the fileWithPath property.
      %   - If @a fileWithPath is omitted, and the current myTopsDataLog
      ...
      % class.  Populates the current instance of myTopsDataLog with any
      % new data.  May also return the new data, in a struct of the same
      % form as myTopsDataLog.getSortedDataStruct().
      % @details
      ...
      % trial structures found in a data file
      % NOTE: this only works if entries in the structures are scalars 
      %  (includig nans)
      function ecodes = parseEcodes(group, fileWithPath)
         ...
         % get the tagged data
         logStruct = myTopsDataLog.getTaggedData(group, fileWithPath);
         ...
         if ~isempty(logStruct)
            ...
            for ii = sort(igroups)'
               ...
               tmp=permute(struct2cell([logStruct(inds).item]),[3 1 2]);
               grdata =cellfun(@double, tmp);
      ...
      function data = getTaggedData(group, fileWithPath)
         ...
         if nargin < 2 || isempty(fileWithPath)
            logStruct = myTopsDataLog.getSortedDataStruct();
         else
            logStruct = myTopsDataLog.readDataFile(fileWithPath);
         end
