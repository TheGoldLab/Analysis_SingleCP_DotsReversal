...
tocast <- unique(data[viewingDuration == 200 | viewingDuration == 300, .(acc, subject, probCP, presenceCP, viewingDuration, vv)])
```
...
for (subj in c("S1", "S2", "S3", "S4", "S5")) {
  ...
  wide[subject == subj & probCP == pcp & presenceCP == "CP", 
       acc_200 := wide[subject == subj & probCP == pcp & presenceCP == "no-CP", acc_200]]
  wide[subject == subj & probCP == pcp & presenceCP == "CP", 
       vv_200 := wide[subject == subj & probCP == pcp & presenceCP == "no-CP", vv_200]]
  }
}
