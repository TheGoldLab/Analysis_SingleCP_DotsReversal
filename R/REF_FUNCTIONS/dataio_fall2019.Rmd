...
t2 <- get_full_data(paste0(DATA_FOLDER, 'full_2019_11_22.csv'))
...
### Subject-by-subject
```{r}
crossover <- t2[coh_cat=="th",`:=`(dirAcc=sum(dirCorrect)/.N, numTrials=.N), by=.(coh_cat, duration, probCP, subject)]
crossover[,`:=`(ci=1.96*sqrt(dirAcc * (1-dirAcc) / numTrials))]
pd <- position_dodge(5)
...
acc0 <- t2[coh_cat=="th",`:=`(dirAcc=sum(dirCorrect)/.N, numTrials=.N), by=.(coh_cat, presenceCP, duration, probCP, subject)]
acc0[,`:=`(ci=1.96*sqrt(dirAcc * (1-dirAcc) / numTrials))]
pd <- position_dodge(5)
