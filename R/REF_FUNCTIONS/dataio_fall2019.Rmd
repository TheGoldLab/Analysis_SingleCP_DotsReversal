...
# Logistic Fits
## With `quickpsy`
Quickpsy allows us to fit lapses.
```{r}
# install.packages("quickpsy")
library(quickpsy)
```
```{r}
noCP_data <- full_data[presenceCP == "noCP"]
noCP_data[, scoh := coherence]
noCP_data[direction == "left", scoh := -coherence]
noCP_data[dirChoice == "right", choice := 1]
noCP_data[dirChoice == "left", choice := -1]
```

```{r}
# With bootstrap="nonparametric" and 3 subjects, the following line takes 1:30 min to execute
fit <- quickpsy(noCP_data, scoh, choice, 
                lapses = FALSE, 
                fun = logistic_fun, 
                grouping = .(subject, duration), 
                bootstrap = "none")
```

```{r, fig.width=12, fig.height=9}
plotcurves(fit, xpanel = duration, ypanel = subject) + 
...
dt1 <- vector(mode = "list", length = numSubjects*numVD)  # for choice proportion data
dt2 <- vector(mode = "list", length = numSubjects*numVD)  # for fitted curves
...
```{r}
new_data[subject == 12 & duration == "600", .(scoh, numTrials)]
```
...
```{r, fig.width=13.5, fig.height=14}
...
ggplot(unique(cpd[coh_cat != "0",.(cpAcc, coh_cat, duration, probCP, ci, subject)]), 
       aes(x=probCP, y=cpAcc, col=coh_cat, group=coh_cat)) + geom_point(size=5, position=pd) + geom_line(size=1) + facet_grid(subject~duration) +
   geom_hline(yintercept = c(0.5,1), linetype='dashed') +
  scale_color_manual(values=cbbPalette[c(2,3)]) + 
