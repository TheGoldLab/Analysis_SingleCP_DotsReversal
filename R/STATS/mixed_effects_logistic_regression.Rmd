...
```{r}
# example logistic fit for single session, after lapse has been estimated 
...
# fetch lapse value
lapse <- lapse_data[duration == dur & probCP == pcp & subject == subj, lapse][1]

raw_data <- full_data[duration == dur & probCP == pcp & subject == subj]
...
dt1 <- vector(mode = "list", length = numSubjects)  # for choice proportion data
dt2 <- vector(mode = "list", length = numSubjects)  # for fitted curves
