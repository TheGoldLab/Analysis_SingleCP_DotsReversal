...
sde.sim(t0 = cp+.01, T = cp+short_viewing_duration, X0 = 0, N = num_time_points_short-1, drift=new_drift,
        sigma=s, sigma.x=0, method = "euler",M=num_traj) -> ZeroResetmodel2
...
highh_reset_vals <- reset_evidence(all_end[num_traj/2+1], highh)
...
means_df <- data.frame("time" = tt,
                       "PA" = meanPA,
                       "lowLeak" = meanLinLow,
                       "HighLeak" = meanLinHigh,
                       "ZeroReset" = meanZeroReset,
                       "LowHreset" = meanLowhReset,
                       "HighHreset" = meanHighhReset)
```
...
X <- as.data.table(X)
X[,`:=`(modelClass="ResetAtCP")]
X[model=="PA",modelClass:="Perfect Accum."]
X[model=="lowLeak" | model=="HighLeak", modelClass:="Leak"]
X[,modelClass:=factor(modelClass, levels = c("Perfect Accum.", "Leak", "ResetAtCP"))]
cp_time=0.2
