...
meanPA <- function(ttt, drift, hasCP, start_offset){
  if (hasCP) {
    ...
    return(start_offset+c(drift * ttt[1:trunc_ix], -drift*ttt[-(1:trunc_ix)]))
  } else {
...
meanReset <- function(ttt, hasCP, reset_func){
  if (hasCP) {
    trunc_ix <- last_ix_before_cp(ttt)
    first_half <- meanPA(ttt[1:trunc_ix], drift, FALSE, 0)
    start_pt <- reset_func(tail(first_half, 1))
    return(c(first_half, meanPA(ttt[-(1:trunc_ix)]-cp, -drift, FALSE, start_pt)))
  } else {
    return(meanPA(ttt, drift, FALSE, 0))
  }
}
...
means_df <- data.frame("time" = tt,
                       ...
                       "HighLeakcp" = meanLinHigh(tt, d0, TRUE, highLeak),
                       ...
                       "lowLeaknoCP" = meanLinLownocp(tt, d0, FALSE, lowLeak),
                       "HighLeaknoCP" = meanLinHighnocp(tt, d0, FALSE, highLeak),
