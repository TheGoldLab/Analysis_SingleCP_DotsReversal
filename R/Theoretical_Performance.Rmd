...
baseC <- seq(.2, 4, .1)
baset <- seq(.1,.4,.001)#c(seq(.1, .2, .01), seq(.201,.25,.001), seq(.26,.4, .01))
...
data <- data.table(C, t, norm_ncp, norm_cp, lowleak_ncp, highleak_ncp, lowleak_cp, highleak_cp, dumbr_cp, dumbr_ncp)



#convert to long format
long_data = melt(data, id.vars = c("C", "t"),
                measure.vars = c("norm_ncp", "norm_cp", "lowleak_ncp", "highleak_ncp", "lowleak_cp", "highleak_cp", "dumbr_cp", "dumbr_ncp"))
...
long_data[,model:=factor(model, levels = c("DDM", "Low Leak", "High Leak", "DumbR"))]
