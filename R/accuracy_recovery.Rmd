...
count[,`:=`(t=viewingDuration/1000, lower=accuracy - ci, upper=accuracy + ci)]
```

```{r, fig.width=12, fig.height=15}
pd <- position_dodge(.1) # move them .05 to the left and right
ggplot(count, aes(x=probCP, y=accuracy, col=presenceCP, group=presenceCP)) + 
  ...
  facet_grid(subject ~ viewingDuration) + theme(text=element_text(size=20)) + 
